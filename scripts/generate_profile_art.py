...
def section_header(theme: Theme, index: int, title: str, label: str) -> str:
  ...
  <path d="M20 25V16h9M940 67v9h-9" fill="none" stroke="{theme.accent}" stroke-width="1.5"/>
  <text x="30" y="56" fill="{theme.accent}" font-family="{MONO}" font-size="16" font-weight="700" letter-spacing="1.4">{number}</text>
  <text x="112" y="59" fill="{theme.title}" font-family="{DISPLAY}" font-size="28" font-weight="700">{_esc(title)}</text>
  ...
  <path d="M112 70H928" stroke="{theme.border}"/><path d="M112 70h96" stroke="{theme.system}" stroke-width="2"/>
</svg>\n'''
...
def cta(theme: Theme, kind: str) -> str:
    ...
    return f'''<svg xmlns="http://www.w3.org/2000/svg" width="240" height="56" viewBox="0 0 240 56" role="img" aria-labelledby="title desc"><title id="title">{title}</title><desc id="desc">Terminal-style {kind} action.</desc><rect x=".5" y=".5" width="239" height="55" rx="13.5" fill="{theme.bg2}" stroke="{theme.border}"/>{_brand_icon(kind)}<path d="M68 13v30" stroke="{theme.system}" stroke-width="2"/><text x="84" y="24" fill="{theme.muted}" font-family="{MONO}" font-size="8.5" font-weight="700" letter-spacing="1.1">{eyebrow}</text><text x="84" y="41" fill="{theme.title}" font-family="{DISPLAY}" font-size="14" font-weight="700"{fit}>{action}</text><path d="M217 28h10m-4-4 4 4-4 4" fill="none" stroke="{theme.accent}" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>\n'''
...
def contact_cell(theme: Theme, key: str) -> str:
    ...
    return f'''<svg xmlns="http://www.w3.org/2000/svg" width="{width}" height="72" viewBox="0 0 {width} 72" role="img" aria-labelledby="title desc"><title id="title">{_esc(label.title())}: {_esc(value)}</title><desc id="desc">Open Ahmad Mujtaba's {_esc(label.lower())} contact.</desc><rect x=".75" y=".75" width="{width - 1.5}" height="70.5" rx="12" fill="{theme.bg2}" stroke="{theme.border}" stroke-width="1.5"/><path d="M16 18h24" stroke="{theme.system}" stroke-width="2"/><text x="16" y="31" fill="{theme.muted}" font-family="{MONO}" font-size="9" font-weight="700" letter-spacing="1.2">{label}</text><text x="16" y="53" fill="{theme.title}" font-family="{DISPLAY}" font-size="{size}" font-weight="600">{_esc(value)}</text><path d="M{width - 29} 25h10m-4-4 4 4-4 4" fill="none" stroke="{theme.accent}" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/></svg>\n'''


def contact_aurora(theme: Theme) -> str:
    return f'''<svg xmlns="http://www.w3.org/2000/svg" width="960" height="160" viewBox="0 0 960 160" role="img" aria-labelledby="title desc"><title id="title">Let’s work together</title><desc id="desc">An invitation to collaborate in production AI.</desc><defs><linearGradient id="surface" x2="1"><stop stop-color="{theme.bg}"/><stop offset="1" stop-color="{theme.bg2}"/></linearGradient><radialGradient id="accent-glow"><stop stop-color="{theme.accent}" stop-opacity=".15"/><stop offset="1" stop-color="{theme.accent}" stop-opacity="0"/></radialGradient><radialGradient id="system-glow"><stop stop-color="{theme.system}" stop-opacity=".13"/><stop offset="1" stop-color="{theme.system}" stop-opacity="0"/></radialGradient><pattern id="grid" width="24" height="24" patternUnits="userSpaceOnUse"><circle cx="2" cy="2" r="1" fill="{theme.muted}" opacity=".13"/></pattern></defs><rect x=".75" y=".75" width="958.5" height="158.5" rx="18" fill="url(#surface)" stroke="{theme.border}" stroke-width="1.5"/><rect x="12" y="12" width="936" height="136" rx="13" fill="url(#grid)"/><ellipse cx="355" cy="86" rx="270" ry="110" fill="url(#accent-glow)"/><ellipse cx="650" cy="95" rx="260" ry="105" fill="url(#system-glow)"/><path d="M28 31V20h11M932 129v11h-11" fill="none" stroke="{theme.system}" stroke-width="1.5"/><text x="480" y="52" text-anchor="middle" fill="{theme.system}" font-family="{MONO}" font-size="10" font-weight="700" letter-spacing="2.2">OPEN TO COLLABORATION</text><text x="480" y="91" text-anchor="middle" fill="{theme.title}" font-family="{DISPLAY}" font-size="30" font-weight="700">Let’s work together</text><text x="480" y="121" text-anchor="middle" fill="{theme.text}" font-family="{DISPLAY}" font-size="12">Production AI · Document Intelligence · Agentic Systems</text><circle cx="394" cy="139" r="4" fill="{theme.live}"/><text x="406" y="143" fill="{theme.muted}" font-family="{MONO}" font-size="10">AVAILABLE</text></svg>\n'''

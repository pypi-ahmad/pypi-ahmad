...
def featured_row(theme: Theme, repo: str) -> str:
    ...
    return f'''<svg xmlns="http://www.w3.org/2000/svg" width="960" height="180" viewBox="0 0 960 180" role="img" aria-labelledby="title desc"><title id="title">{_esc(title)}</title><desc id="desc">{_esc(description)} Flow: {_esc(' to '.join(flow))}.</desc>{_defs(theme)}<rect x=".75" y=".75" width="958.5" height="178.5" rx="8" fill="url(#surface)" stroke="{theme.border}" stroke-width="1.5"/><rect x="1" y="1" width="958" height="178" rx="8" fill="url(#rules)"/><path d="M210 20v140M660 20v140" stroke="{theme.border}"/><path d="M18 20v140" stroke="{theme.accent}" stroke-width="3"/><text x="32" y="48" fill="{theme.muted}" font-family="{MONO}" font-size="11" font-weight="700" letter-spacing="1.4">{_esc(category)}</text><text x="32" y="82" fill="{theme.title}" font-family="{DISPLAY}" font-size="24" font-weight="700">{_esc(title)}</text><text x="32" y="144" fill="{theme.accent}" font-family="{MONO}" font-size="11" font-weight="700">VIEW CODE ↗</text><text x="238" y="55" fill="{theme.muted}" font-family="{MONO}" font-size="10" font-weight="700" letter-spacing="1.3">PROJECT / SYSTEM</text>{body}<text x="690" y="31" fill="{theme.muted}" font-family="{MONO}" font-size="9" font-weight="700" letter-spacing="1.2">ARCHITECTURE FLOW</text>{edges}{''.join(nodes)}</svg>\n'''


def contact_cell(theme: Theme, key: str) -> str:
    label, value, _, width = CONTACTS[key]
    size = 12 if len(value) > 19 else 14
    return f'''<svg xmlns="http://www.w3.org/2000/svg" width="{width}" height="72" viewBox="0 0 {width} 72" role="img" aria-labelledby="title desc"><title id="title">{_esc(label.title())}: {_esc(value)}</title><desc id="desc">Open Ahmad Mujtaba's {_esc(label.lower())} contact.</desc><rect x=".75" y=".75" width="{width - 1.5}" height="70.5" rx="5" fill="{theme.bg2}" stroke="{theme.border}" stroke-width="1.5"/><path d="M1 1h{width - 2}" stroke="{theme.accent}" stroke-width="2"/><text x="16" y="28" fill="{theme.muted}" font-family="{MONO}" font-size="9" font-weight="700" letter-spacing="1.2">{label}</text><text x="16" y="53" fill="{theme.title}" font-family="{DISPLAY}" font-size="{size}" font-weight="600">{_esc(value)}</text><path d="M{width - 29} 24h10m-4-4 4 4-4 4" fill="none" stroke="{theme.accent}" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/></svg>\n'''
